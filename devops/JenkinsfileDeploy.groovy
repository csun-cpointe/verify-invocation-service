def gitId = 'github'
def gitRepo = 'https://github.com/csun-cpointe/verify-invocation-service.git'
// Set branch in the Jenkins Job or set here if not variable
def gitBranch = 'refs/heads/${branch}'
def projectDirectory = 'verify-invocation-service'
def jenkinsSteps
// Set URL of ArgoCD server
def argocdUrl = ''
def argocdAppName = 'verify-invocation-service'
def argocdNamespace = 'argocd'
def argocdDestinationServer = 'https://kubernetes.default.svc'
def argocdBranch = '${branch}'
def argocdSyncLabel = 'argocd.argoproj.io/instance=verify-invocation-service'
            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Test Pipeline Invocation authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Test Pipeline Invocation failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test Pipeline Invocation deploying"
                ...
                slackSend color: "good",
                        message: "Test Pipeline Invocation deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test Pipeline Invocation failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test Pipeline Invocation shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test Pipeline Invocation failed to shut down properly"
            throw err
        }
    }
}
